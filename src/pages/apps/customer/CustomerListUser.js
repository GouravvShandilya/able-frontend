...
// import CustomerView from 'sections/apps/customer/DepositorView';
...
import CustomerViewUser from 'sections/apps/customer/CustomerViewUser';
...
const CustomerListUser = () => {
  ...
  async function getAllCustomer() {
    try {
      const da = await axiosServices.get('customer/getAllCustomer');

      setData(da.data.customers);
    } catch (error) {
      console.log(error);
    }
  }
  ...
  const renderRowSubComponent = useCallback(({ row }) => <CustomerViewUser data={data[Number(row.id)]} />, [data]);
...
export default CustomerListUser;
