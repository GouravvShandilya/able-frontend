...
import DepositorView from 'sections/apps/customer/DepositorView';
import AlertCustomerDelete from 'sections/apps/customer/AlertDepositorDelete';
...
function ReactTable({ columns, data, renderRowSubComponent, handleAdd }) {
  ...
  const sortBy = { id: 'full_name', desc: false };
...
const DepositorList = () => {
  ...
  async function fetchData() {
    const data = await axiosServices.get('/admin/allUser');
  ...
  const columns = useMemo(
    () => [
        ...
        Cell: ({ row }) => {
          ...
          return (
            <Stack direction="row" spacing={1.5} alignItems="center">
              ...
              <Stack spacing={0}>
                <Typography variant="subtitle1">{values?.full_name}</Typography>
      ...
      },
      {
        ...
        Cell: ({ value }) => <PatternFormat displayType="text" format="+91 ### ### ####" mask="_" defaultValue={value} />
      },
      {
              ...
              >
                <IconButton
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    handleClose();
                    setCustomerDeleteId(row.values._id);
                  }}
                >
                  <Trash />
                </IconButton>
  ...
  const renderRowSubComponent = useCallback(({ row }) => <DepositorView data={data[Number(row.id)]} />, [data]);
