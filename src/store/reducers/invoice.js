...
export function getInvoiceList() {
  return async () => {
    try {
      const response = await axios.get('/invoice/get_invoiceAdmin');
...
export function getInvoiceInsert(NewLists) {
  return async () => {
    try {
      ...
      const response = await axios.post('/invoice/create_invoiceAdmin', NewLists);
...
export function getInvoiceUpdate(NewLists) {
  return async () => {
    try {
      const response = await axios.post('/invoice/invoiceAdminupdate', { list: NewLists });
...
export function getInvoiceDelete(invoiceId) {
  return async () => {
    try {
      await axios.post('/invoice/delete_invoiceAdmin', { invoiceId });
...
export function getInvoiceSingleList(invoiceId) {
  return async () => {
    try {
      const response = await axios.post('/invoice/single_invoiceAdmin', { invoiceId });
      dispatch(invoice.actions.getSingleList(response.data.invoice));
    } catch (error) {
      dispatch(invoice.actions.hasError(error));
    }
  };
