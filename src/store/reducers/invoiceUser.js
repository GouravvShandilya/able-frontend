...
export function getInvoiceList() {
  return async () => {
    try {
      const response = await axios.get('/invoice/getInvoicesUser');
...
export function getInvoiceInsert(NewLists) {
  return async () => {
    try {
      const response = await axios.post('/invoice/createInvoiceUser', NewLists);
...
export function getInvoiceUpdate(NewLists) {
  return async () => {
    try {
      const response = await axios.post('/invoice/invoiceUserUpdate', { list: NewLists });
...
export function getInvoiceDelete(invoiceId) {
  return async () => {
    try {
      await axios.post('/invoice/deleteinvoiceUser', { invoiceId });
...
export function getInvoiceSingleList(invoiceId) {
  return async () => {
    try {
      const response = await axios.post('/invoice//singleInvoiceAdmin', { invoiceId });
